...
mpnn.compile(opt, log_mse, metrics = [mae, log_mse])

train_size = int(len(out_labels)*0.8)
batch_size = 16
epochs = 50
KERNAL_NUMBER = 'MPNN-006'

mpnn.call({'adj_input' : in_edges_train[:10], 'nod_input': nodes_train[:10]})

######### RUN THE MODEL

filepath="mpnn-models/weights-improvement-{epoch:02d}-{val_mae:.5f}.hdf5"
...
mpnn.load_weights('mpnn-models/weights-improvement-42-0.00402.hdf5')
